...
def predict_tracks(
    ...
    if masks is not None:
        ...
        assert masks.shape[-2:] == images.shape[-2:], f"Mask and image size mismatch. Mask shape: {masks.shape[-2:]}, Image shape: {images.shape[-2:]}"
        images = images * masks
...
def _forward_on_query(
    ...
    if masks is not None:
        query_mask = masks[query_index]
        assert query_mask.shape[-2:] == query_image.shape[-2:], f"Mask and image size mismatch. Mask shape: {query_mask.shape[-2:]}, Image shape: {query_image.shape[-2:]}"

        # Select only keypoints inside nonzero mask region
        x = query_points[0, :, 0].long().clamp(0, query_mask.shape[-1] - 1)
        y = query_points[0, :, 1].long().clamp(0, query_mask.shape[-2] - 1)
        valid_mask_pts = query_mask[0, y, x] > 0.5
        query_points = query_points[:, valid_mask_pts]

        if query_points.shape[1] == 0:
            print(f"[Mask Warning] Frame {query_index} has no valid keypoints after masking.")
            return np.zeros((0, width, 2)), np.zeros((0, width)), None, None, None
