...
Purpose: Download PyPI wheels & generate `blender_manifest.toml` for Mosplat Blender add-on.
"""
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Download PyPI wheels & generate `blender_manifest.toml` for Mosplat Blender add-on.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    current_dir: Path = Path(os.path.dirname(os.path.abspath(__file__)))
    default_addon_path: Path = Path(
        os.path.abspath(os.path.join(current_dir, "..", "mosplat_blender"))
    )
    parser.add_argument(
        "-p",
        "--path",
...
def download_pypi_wheels(addon_dir, wheels_dir, version):
    ...
    requirements_txt_file: Path = Path(os.path.join(addon_dir, "requirements.txt"))
...
def generate_blender_manifest_toml(addon_dir, wheels_dir):
    ...
    manifest_txt_file: Path = Path(os.path.join(addon_dir, "blender_manifest.txt"))
    print(f"`blender_manifest.txt` File Path: {manifest_txt_file}")

    manifest_toml_file: Path = Path(os.path.join(addon_dir, "blender_manifest.toml"))
...
def main():
    ...
    wheels_dir: Path = Path(os.path.join(args.path, "wheels"))
